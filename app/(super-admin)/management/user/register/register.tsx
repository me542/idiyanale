...
export default function AddUserModal({
  ...
  try {
    ...
    // Insti-admins are scoped to their own institution.
    // Super admins have no institution_id and should see all of them.
    const scopedInstitutions = jwt.institution_id
      ? institutionList.filter(
          (institution) =>
            institution.institution_id === jwt.institution_id
        )
      : institutionList;

    setInstitutions(scopedInstitutions);

    // Only auto-select when the user is scoped to exactly one institution.
    if (jwt.institution_id && scopedInstitutions.length > 0) {
      setForm((current) => ({
        ...current,
        institution: String(scopedInstitutions[0].institution_id),
      }));
    }
  } catch (err) {
...
if (res?.ret_code && res.ret_code !== "200") {
